...
const TopicList = part((props) => {
  return part`
  <ul>
    ${topics.map((topic) => {
      let href = props.match.add.url(topic.id);
      return part`<li><a ${{ href }} ${link()}>${topic.name}</a></li>`;
    })}
  </ul>
  <hr/>
  ${Router({ routes: topicRoutes })}`;
});

const Topic = part((props) => () => {
  ...
  return part`
    ...
    ${topic.resources.map((res) => {
      const href = match.add.url(res.id);
      return part`<li><a ${{ href }} ${link()}>${res.name}</a></li>`;
    })}
  </ul>
  <hr/>
  ${Router({ routes: resourceRoutes })}
  `;
});

const ResourceList = part((props) => () => {
  const match = props.match;
