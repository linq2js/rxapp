...
import objectEqual from "../core/objectEqual";
...
let Router = createComponent((props, { update }) => {
  ...
  function matchRoute(pattern) {
    return matcher(pattern, history.location.pathname);
  }
  ...
  return () => {
    ...
    if (
      ...
      objectEqual(prev.routes, routes)
    )
      return prev.result;

    prev = { routes, pathname: location.pathname, state: location.state };
    ...
    return (prev.result = RouterProvider({
